...
export const adminPageRoutes = [
   ...
   <ProtectedRoute
      key={Math.random()}
      path={ADMIN_PAGE_SETTINGS}
      component={Settings}
   />,
   <ProtectedRoute
      key={Math.random()}
      path={ADMIN_PAGE_WASHINGMACHINE_ACTIVE}
      component={WashingMachineActive}
   />,
   <ProtectedRoute
      key={Math.random()}
      path={ADMIN_PAGE_WASHINGMACHINE_INACTIVE}
      component={WashingMachineInactive}
   />,
   <ProtectedRoute
      key={Math.random()}
      path={ADMIN_PAGE_UPDATE_SLOTS}
      component={UpdateWashingSlotsDetails}
   />
]
